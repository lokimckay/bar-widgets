...
local widgetSlug = "gfx_widget_name"
...
local function CheckCompat() if not initGL4() then return false end end

-- https://beyond-all-reason.github.io/spring/ldoc/modules/LuaHandle.html#Initialize
function widget:Initialize()
    DebugLog(widgetName .. " widget enabled")
    if not CheckCompat() then widgetHandler:RemoveWidget() end
end
...
local function initGL4()
    ...
    shader = LuaShader({vertex = vsSrc, fragment = fsSrc}, widgetSlug)
