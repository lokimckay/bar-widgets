...
local function IsTechFactory(unitDefID)
	...
	if unitDefID == nil or unitDef == nil then
		return false
	end
...
local function IsOwnedByThisPlayer(unitTeam)
	...
	return Spring.GetPlayerInfo(Spring.GetMyPlayerID()) == Spring.GetPlayerInfo(
		unitTeam
	)
end
...
local function ShouldNotify(messageType, unitDefID, unitTeam)
	...
	if not IsOwnedByThisPlayer(unitTeam) then
		return false
	end

	-- Check if message has already been sent
	if messagesSent[messageType] then
		return false
	end

	-- Check if config has disabled this message type
	if messageType == "start" and not notifyStart then
		return false
	elseif messageType == "finish" and not notifyFinish then
		return false
	elseif messageType == "reclaim" and not notifyReclaim then
		return false
	end
...
local function GetShortName(name)
	if name == "Advanced Bot Lab" then
		return "Bots"
	elseif name == "Advanced Vehicle Plant" then
		return "Veh"
	elseif name == "Advanced Aircraft Plant" then
		return "Air"
	elseif name == "Advanced Shipyard" then
		return "Sea"
	else
		return name
	end
...
local function GetFaction(unitDefID)
	...
	if substring == "cor" then
		return "Core"
	elseif substring == "arm" then
		return "Arm"
	else
		return "unknown"
	end
...
local function HydrateMessage(rawMessage, techlevel, faction, factory, cost)
	return rawMessage:gsub("<LVL>", techlevel or ""):gsub(
		"<FACTION>",
		faction or ""
	):gsub("<TYPE>", factory or ""):gsub("<COST>", cost or "")
end
...
local function AddMarker(unitID, message, messageType)
	...
	if messageType == "start" then
		offset = -100
	elseif messageType == "finish" then
		offset = -50
	end
...
local function IsLastMessage(messageType)
	if notifyReclaim then
		return messageType == "reclaim"
	elseif notifyFinish then
...
local function Notify(messageType, rawMessage, unitID, unitDefID, unitTeam)
	...
	local message =
		HydrateMessage(rawMessage, techlevel, faction, factory, conCost)
...
function widget:UnitFinished(unitID, unitDefID, unitTeam)
	if cancelIfAllyTechs then
		CheckIfAllyTeched(unitID, unitDefID, unitTeam)
	end
	...
	if ShouldNotify("finish", unitDefID, unitTeam) then
		Notify(
			"finish",
			offerToSell and messages["finish"] or messages["finishNoSell"],
			unitID,
			unitDefID,
			unitTeam
		)
...
function widget:Initialize()
	if (Spring.IsReplay() or Spring.GetSpectatingState()) then
		widgetHandler:RemoveWidget()
	end -- Disable the widget if spectating
