...
local function AddGhost(x, y, z, unitDefID, teamID)
	...
	if ghostShapeID ~= nil then
		RemoveGhost()
	end -- Remove existing ghost if it exists
	ghostShapeID =
		WG.DrawUnitShapeGL4(
			unitDefID,
			x,
...
local function GetWeaponRanges(unitDefID)
	...
	for idx = 1, #weapons do
		...
		if weapon.range then
			if weapon.canAttackGround then
				table.insert(groundRanges, weapon.range)
			end
...
local function ParseSelection(selection, sortBy)
	if not selectionStale then
		return parsedUnits
	end -- Return existing global values if selection has not changed
	local allUnits = {}
	...
	local function AddUnitRecord(t, defID, unitID)
		...
		if existingRecord then
			...
			local maxGroundRange =
				#groundRanges > 0 and math.max(unpack(groundRanges)) or 0
	...
	for i, unitID in ipairs(selection) do
		...
		if isTransport then -- selected unit is a transport - we only care about any loaded units
			...
			-- selected unit is not a transport
...
function widget:Update()
	...
	if ghost == nil then
		...
		SetActive(false)
		return
	end -- Return if no units selected or no ranges to draw
end

-- https://springrts.com/wiki/Lua:Callins#KeyPress
function widget:KeyPress(key, mods, isRepeat)
	if isRepeat then return end -- Ignore if key is being held down
	if key == 27 and isActive then
		SetActive(false)
	end -- if escape is pressed, deactivate rangefinder
end

-- https://springrts.com/wiki/Lua:Callins#MousePress
function widget:MousePress()
	if isActive and cancelOnMouseClick then
		SetActive(false)
	end -- if any mouse button is clicked, deactivate rangefinder
end
