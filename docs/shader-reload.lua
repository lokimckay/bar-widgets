...
local function checkShaderUpdates(
vssrcpath,
	fssrcpath,
	gssrcpath,
	shadername,
	...
	if lastshaderupdate == nil or Spring.DiffTimers(
		Spring.GetTimer(),
		lastshaderupdate
		...
		if vsSrcNew == shaderSourceCache.vsSrc and fsSrcNew == shaderSourceCache.fsSrc and gsSrcNew == shaderSourceCache.gsSrc then
			...
			if vsSrcNew then
				...
				vsSrcNew =
					vsSrcNew:gsub(
						"//__DEFINES__",
						LuaShader.CreateShaderDefinesString(shaderConfig)
					)
			end
			if fsSrcNew then
				fsSrcNew =
